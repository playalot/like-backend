...
import models.{ JudgePost, JudgePostsComponent, Page }
...
class DashboardService @Inject() (protected val dbConfigProvider: DatabaseConfigProvider) extends HasDatabaseConfigProvider[JdbcProfile]
    ...
    with JudgePostsComponent {

  import driver.api._

  def list(page: Int = 0, pageSize: Int = 36): Future[Page[(Post, Seq[(Long, String, Int)])]] = {
  ...
  def judgePost(postId: Long, judge: Boolean): Future[Unit] = {
    db.run(justPosts += JudgePost(postId, judge)).map(_ => ())
  }

  def getNotJudgedPosts(startId: Option[Long]): Future[Seq[(Long, String)]] = {
    val query = if (startId.isDefined) {
      sql"SELECT id, content FROM post where id < ${startId.get} AND id NOT IN (SELECT id FROM judge_posts) ORDER BY id DESC limit 100"
    } else {
      sql"SELECT id, content FROM post where id NOT IN (SELECT id FROM judge_posts) ORDER BY id DESC limit 100"
    }
    db.run(query.as[(Long, String)])
  }
